...
const quizDataMedium = [
  ...
  { question: "조례안에 예산이 수반되는 경우, 발의 시 비용추계서를 첨부해야 한다.", correctAnswer: "O", explanation: "「경기도의회 회의규칙」제25조(의안의 제출과 발의)에 따라, 그 의안의 시행에 수반될 것으로 예상되는 비용에 대한 추계서와 이에 상응하는 재원조달방안에 관한 자료를 의안에 첨부하여야 합니다." },
  ...
  { question: "정례회는 매년 2회 개최하며, 예산안·결산안 심의, 행정사무감사 등이 열리는 경우가 많다.", correctAnswer: "O", explanation: "정례회는 예산·결산, 행정사무감사가 주로 열리는 중요한 회기입니다." },
  ...
  { question: "경기도의회 회의는 원격영상회의 방식으로도 진행할 수 있다.", correctAnswer: "O", explanation: "「경기도의회 회의규칙」 제62조에 관련 근거가 있으며, 의장과 교섭단체 대표의원의 합의로 원격영상회의가 가능합니다.(예: 감염병 상황 등)" },
...
const quizDataHard = [
  ...
  { question: "본회의 의결로 의회운영위원회에 안건이 재회부되면, 의회운영위원회는 그 안건을 다시 본회의에 상정하지 않아도 된다.", correctAnswer: "X", explanation: "재회부된 안건은 다시 심사 후, 본회의에 재보고·재상정해야 의결 절차가 종결됩니다." },
  ...
  { question: "경기도의회는 의원 전원으로 구성된 본회의 외에도, 의회운영위원회를 제외한 다른 상임위원회는 모두 임시적으로만 운영된다.", correctAnswer: "X", explanation: "의회운영위원회 외에도 기획재정위원회, 보건복지위원회 등 다수의 상임위원회가 정식으로 상시 운영됩니다." },
...
function startQuiz(difficulty) {
  console.log('Quiz started with difficulty:', difficulty);  // 디버깅용 로그
  ...
  } else if (difficulty === 'euihoe') { // 의회교실 난이도
    currentQuizData = getRandomFive(quizDataEuihoe); // 5문항만 추출
  }

  console.log('currentQuizData after selection:', currentQuizData);  // 디버깅용 로그
...
// 의회교실 난이도는 5문항만 추출
function getRandomFive(fullArray) {
  // shallow copy 후 shuffle, 앞에서 5개 slice
  let copied = [...fullArray];
  shuffleArray(copied);
  return copied.slice(0, 5);
}
